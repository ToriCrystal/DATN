                            ...
                            <div class="card-body">
                                ...
                                <form class="forms-sample" action="{{ route('admin.newsarticlesupdate', $newsarticle->id) }}"
                                    ...
                                    <img id="preview" style="max-width: 100%; max-height: 200px; margin-top: 10px;"
                                        src="{{$newsarticle->post_thumbnail }}" />

                                    <div class="form-group pt-2">
                                        <label for="published_at">Ngày xuất bản</label>
                                        <input type="date" class="form-control" id="published_at" name="published_at"
                                            min="{{ date('Y-m-d') }}" value="{{ $newsarticle->published_at }}">
                                    </div>
                                    <div class="form-group pt-2">
                                        <label for="isHot">Đặc sắc</label>
                                        <select class="form-control" id="isHot" name="isHot">
                                            <option value="0" @if ($newsarticle->isHot == 0) selected @endif>Không
                                            </option>
                                            <option value="1" @if ($newsarticle->isHot == 1) selected @endif>Có
                                            </option>
                                        </select>
                                    </div>
                                    <div class="form-group pt-2">
                                        ...
                                        <select class="form-control" id="status" name="status">
                                            <option value="0" @if ($newsarticle->status == 0) selected @endif>Ẩn
                                            </option>
                                            <option value="1" @if ($newsarticle->status == 1) selected @endif>Hiện
                                            </option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="meta_title" name="meta_title"
                                            value=" {{ $newsarticle->meta_title }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="meta_description">Mô tả meta</label>
                                        <input type="text" class="form-control" id="meta_description"
                                            name="meta_description" value="{{ $newsarticle->meta_description }}">
                                    </div>

                                    <div class="form-group">

                                        <label for="editor">Nội dung</label>
                                        <textarea class="form-control" name="post_content" id="editor" rows="4" style="padding: 10px;">{!! $newsarticle->post_content !!}</textarea>
                                    </div>
